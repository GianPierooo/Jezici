...
|---|---|---|---|
| **VOZ del cliente en INGLÉS fijo** (feedback real "voz correspondiente al idioma") | **P0/P1 core** | ✅ **REPARADO (2026-07-02b)** | **Bug REAL confirmado en código:** `word_tts_web.dart` (TTS al tocar una ficha en word_bank/reorder) hardcodeaba `lang='en-US'` y `speaking_exercise.dart` el reconocedor `localeId='en_US'` → en pt/fr/it la voz/reconocimiento eran INGLÉS (afecta 3 de 4 cursos en el loop central). Fix de CLASE: `SpeechLang` (estático) sigue el idioma META del curso activo (en-US/pt-BR/fr-FR/it-IT), fijado en `HomeShell` desde `activeCourseTargetProvider`; `WordTts.speak` usa `SpeechLang.tts`, el speaking usa `SpeechLang.stt`. El audio pre-generado (MP3 listening/speaking, tl=fr/it) YA era correcto; el bug era solo el Web Speech del cliente. Verificado: unit test del mapeo + cliente real `get_courses` da `target` correcto por curso activo (en/pt/fr/it) + analyze/test 90/90. (La voz concreta depende de que el dispositivo tenga voz para ese idioma; degrada con gracia.) |
...
| **Conversar hardcodeado en inglés** | **P1** | ✅ **REPARADO (2026-07-02c)** | Los 6 topics tenían model+tips solo en inglés → pt/fr/it veían inglés. Ahora `ConvTopic.models` = mapa por idioma META (en/pt/fr/it), servido con `activeCourseTargetProvider`+`modelFor(lang)` (fallback en). Model+tips autorados por profesores nativos (pt-BR/fr/it). Reconocedor de voz de Conversar → `SpeechLang.stt` (era 'en_US'). Unit test (6×4×3 + fallback) + cliente real (target por curso: en/pt/fr/it). |
...
| **Bugs reportados por usuarios reales** (ahora legibles) | — | 📋 | ✅ **"Voz correspondiente al idioma"** (perfil, 2026-07-02) → **era REAL, reparado** (ver 1ª fila: TTS/reconocedor en inglés fijo). · "El radar no carga" (perfil, 2026-06-17) → **no reproduce** hoy (get_skill_mastery=4 skills, get_profile OK con cliente real; anterior al rework de perfil). |
