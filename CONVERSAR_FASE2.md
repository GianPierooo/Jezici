...
| **P1 — Cimientos de seguridad** ✅ **LIVE (mig 146, 2026-07-11)** | age gate 18+ real (año→tier, `jz_is_adult_user` fail-closed, pantalla neutral + red de seguridad) · `blocks`/`mutes`/`moderation_actions` + `reports` completo · RPCs block/mute/report (rate-limited, no-self, no-sancionado) · cola admin `get_reports`/`mod_apply`/`resolve_report` (am_i_admin) · RLS estricta (escritura solo por RPC, **bloqueo cortado en la RLS de `social_profiles` en ambas direcciones**, 18+ en RLS, revocados los write-grants directos) · verificado cliente real. **Falta legal para ABRIR la Ola 1.** | L | **Abogado** (revisar ToS/privacidad social+menores) — BLOQUEO para abrir cualquier función social |
| **P2 — Ola 1 (social async cerrado)** 🔶 **PARCIAL LIVE (mig 147, 2026-07-11)** — CERRADO (allowlist `social_beta`): amigos por código (A1) · chat texto 1:1 con filtro de contacto + rate limit (A2) · corrección entre amigos (A4) · racha con amigos derivada de `daily_goals` (A5) · report/block en chat. Cliente: sección "Amigos" en Conversar (gate `jz_social_access`) + FriendsScreen + ChatScreen (Realtime). Verificado cliente real. **Re-encolado:** A3 notas de voz (recorder web + Storage RLS), A6 co-op, F5/F7. | L | — (solo Supabase); **APERTURA bloqueada por legal UGC** |
