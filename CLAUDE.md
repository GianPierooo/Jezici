...
1. **Historias/inmersión B1 para fr/it/de/nl/pt** (hoy todas topan en A1; los 5 cursos ya llegan a B2). RETOME EXACTO:
   5 agentes nativos autoran `story_<code>_2.json` (order_index=2, cefr_level="B1"): 7 segments{en:<texto idioma meta>,
   es:<traducción>}, subtitle/emoji/intro, glossary ~6, questions = 5 MC {prompt, options[3], value} → `gen_stories.py
   20260705120125 story_fr_2.json story_it_2.json story_de_2.json story_nl_2.json story_pt_2.json` → apply → audio
   `gen_story_audio_multi.py` (tl por idioma, 35 audios) → `verify_stories_multi.py` (course-scoped, get_story sin
   correct_answer, submit_story 42501). Molde: `story_<code>_1.json` (A1) existentes. Luego B2 stories.
2. **C1 es→fr/it/de/nl** (topan en B2; siguiente escalón real) — o **C1 es→pt**. Reserva STAMP + `('<code>','c1')` en
...
6. **Diferidos menores:** 2ª historia/inmersión por idioma + historias B2; imágenes referenciales
