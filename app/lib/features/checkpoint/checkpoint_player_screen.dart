...
/// Examen del checkpoint: cronómetro visible, set aleatorizado, SIN pistas ni
/// feedback por ítem (resultados solo al final). Auto-envía al expirar el tiempo.
class CheckpointPlayerScreen extends ConsumerStatefulWidget {
  const CheckpointPlayerScreen({
    super.key,
  ...
  @override
  ConsumerState<CheckpointPlayerScreen> createState() => _CheckpointPlayerScreenState();
}

class _CheckpointPlayerScreenState extends ConsumerState<CheckpointPlayerScreen> {
  ...
  void _next() {
    _answers.add({'item_id': _items[_index].id, 'answer': _jsonAnswer(_answer.value)});
  ...
  Future<void> _submit() async {
    ...
    _submitting = true;
    final timeTaken = widget.data.timeLimitSec - _remaining;
    showDialog<void>(
      context: context,
    ...
    try {
      final result = await ref
          ...
          .submitCheckpoint(widget.lesson.id, _answers, timeTaken);
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (_) => CheckpointResultScreen(
            result: result,
                    ...
                    child: Row(
                      ...
                      children: [
                        Icon(Icons.timer_outlined,
                            size: 17, color: low ? AppColors.hearts : AppColors.primary),
                        const SizedBox(width: 5),
                        Text(_timeText,
                            style: TextStyle(
                                fontSize: 15,
                                fontWeight: FontWeight.w900,
                                color: low ? AppColors.hearts : AppColors.primary)),
                      ],
                    ),
                  ),
                  const Spacer(),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    KeyedSubtree(
                      ...
                      child: buildExerciseWidget(context, item, _answer, false),
                    ),
                  ],
                ),
              ),
