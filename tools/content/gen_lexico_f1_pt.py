# -*- coding: utf-8 -*-
"""LÉXICO Fase 1 · PORTUGUÉS — autoría de vocabulario NUEVO (LEXICO_PLAN §3/§4).
...
uso: python gen_lexico_f1_en.py  → escribe la migración + _lexpt/audio_targets.json
"""
import json, re, uuid, unicodedata, os

NS = uuid.uuid5(uuid.NAMESPACE_URL, 'jezici/lexico_f1/pt')
CID = '20000000-0000-0000-0000-000000000002'
...
def main():
    taught = set(json.load(open('_lexpt/taught_pt.json', encoding='utf-8')))
    words = json.load(open('_lexpt/final.json', encoding='utf-8'))

    # el campo 'cloze' ya viene de candidates (forma exacta presente en la oración)
    for w in words:
        w['cloze'] = w.get('cloze') or w['word']
        w['nw'] = norm(w['word'])
    ...
    # ── migración ──
    out = ["-- LÉXICO Fase 1 · PORTUGUÉS — vocabulario NUEVO (LEXICO_PLAN §3/§4).",
    ...
    mig = '../../supabase/migrations/20260718120170_lexico_f1_pt.sql'
    open(mig, 'w', encoding='utf-8').write('\n'.join(out))
    json.dump(audio_targets, open('_lexpt/audio_targets.json', 'w', encoding='utf-8'), ensure_ascii=False)
