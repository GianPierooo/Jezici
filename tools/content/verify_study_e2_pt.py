# -*- coding: utf-8 -*-
"""ESTUDIAR · E-2 (PORTUGUÉS A1+A2) — verificación con CLIENTE REAL (JWT):
...
def main():
    ...
    tok, uid = V.mk_user('studye2pt@test.jezici.dev')
    ...
    check(t is not None and t.get('cefr_level') == 'A1', 'pt U1 (A1) devuelve sesión de estudio')
    ...
    check(all(c == 200 for c in codes), 'audio pt U1 HEAD 200 -> %s' % codes)
    ...
    check(t12.get('cefr_level') == 'A2' and all(c == 200 for c in codes12),
          'pt U12 (A2) sirve sesión y audio HEAD 200 -> %s' % codes12)
    ...
    bank = I.run("""
      select st.unit_order, it->>'id' id, it->>'answer' ans
       ...
       where st.course_id='%s' order by st.unit_order;""" % PT)
    by_unit = {}
    for b in bank:
    ...
    check(bad == 0, 'las %d respuestas correctas de pt se ACEPTAN (0 castigadas)' % total)
