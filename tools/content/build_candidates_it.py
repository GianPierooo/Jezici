# -*- coding: utf-8 -*-
"""LÉXICO Fase 1 · ITALIANO — guardas deterministas sobre los 10 theme_*.json.
Produce _lexit/candidates.json (palabras que pasan) + 5 review_in_*.json para los
revisores adversariales. Guardas:
  1) la palabra/cloze aparece EXACTA en la oración (resuelve cloze; None si no)
  2) dedup vs lo que it ya enseña (taught_it.json, raíz sin artículo)
...
ARTS = ['il ', 'lo ', 'la ', "l'", 'i ', 'gli ', 'le ', 'un ', 'uno ', "un'", 'una ',
        'el ', 'los ', 'las ', 'la ', 'un ', 'una ']
...
def main():
    ...
    json.dump(cands, open('_lexit/candidates.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
    ...
    for k in range(n):
        json.dump(chunks[k], open('_lexit/review_in_%d.json' % k, 'w', encoding='utf-8'),
