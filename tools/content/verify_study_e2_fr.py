# -*- coding: utf-8 -*-
"""ESTUDIAR · E-2 (FRANCÉS A1+A2) — verificación con CLIENTE REAL (JWT):
...
def main():
    ...
    units = I.run("""select id, order_index from units
                     ...
                     order by order_index;""" % FR)
    by_order = {u['order_index']: u for u in units}

    tok, uid = V.mk_user('studye2fr@test.jezici.dev')
    V.rpc(tok, 'submit_age_gate', {'p_birth_year': 1990})
    V.rpc(tok, 'set_active_course', {'p_course_id': FR})
    ...
    check(t is not None and t.get('cefr_level') == 'A1', 'fr U1 (A1) devuelve sesión de estudio')
    ...
    check(all(e.get('text') for e in t['examples']),
          'los ejemplos traen la clave CANÓNICA text (francés)')
    codes = [head(e.get('audio_url') or '') for e in t['examples']]
    check(all(c == 200 for c in codes), 'audio fr U1 HEAD 200 -> %s' % codes)
    ...
    check(t12.get('cefr_level') == 'A2' and all(c == 200 for c in codes12),
          'fr U12 (A2) sirve sesión y audio HEAD 200 -> %s' % codes12)
    ...
    bank = I.run("""
      select st.unit_order, it->>'id' id, it->>'answer' ans, it->>'type' ty
       ...
       where st.course_id='%s' order by st.unit_order;""" % FR)
    by_unit = {}
    for b in bank:
        by_unit.setdefault(b['unit_order'], []).append(b)
    total = bad = 0
    for uo, items in by_unit.items():
        u = I.run("select id from units where course_id='%s' and order_index=%d;" % (FR, uo))[0]
        r = V.rpc(tok, 'submit_study_quiz', {
            'p_unit_id': u['id'],
            'p_answers': [{'id': i['id'], 'answer': i['ans']} for i in items]})
        total += r['graded']
        bad += r['graded'] - r['correct']
    check(bad == 0, 'las %d respuestas correctas de fr se ACEPTAN (0 castigadas)' % total)
    ...
    for cid, nom in ((EN, 'INGLÉS'), (PT, 'PORTUGUÉS')):
        u1 = I.run("select id from units where course_id='%s' and order_index=1;" % cid)[0]
        to = V.rpc(tok, 'get_study_theory', {'p_unit_id': u1['id']})
        check(to is not None and to.get('title') != t.get('title'),
              'AISLAMIENTO: la U1 del %s sigue sirviendo SU sesión (%r)' % (nom, (to or {}).get('title')))
    t25 = V.rpc(tok, 'get_study_theory', {'p_unit_id': by_order[25]['id']})
    check(t25 is None, 'fr U25 (C1, sin tips) sigue null -> "teoría en camino"')
