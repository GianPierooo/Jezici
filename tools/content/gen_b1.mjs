// Genera la migración SQL del currículo B1 (Unidades 13–18) a partir del JSON
// autorado (b1_units.json, salida del workflow b1-author-content), normalizando
// al formato del grader y validando ANTES de emitir. Clon de gen_a2.mjs con:
//  · cefr_level='B1', order_index = U (13..18).
//  · ids propios (c3NNNNNN / b1bNNNNN), sin colisión con A1/A2.
//  · dificultad B1 en [0.35, 0.65].
//  · content_items con ON CONFLICT DO UPDATE (idempotente → re-sembrar aplica fixes).
import fs from 'node:fs';

const IN = process.argv[2] || 'C:/Users/gianp/Desktop/Jezici/tools/content/b1_units.json';
const OUT = process.argv[3] || 'C:/Users/gianp/Desktop/Jezici/supabase/migrations/20260618130043_seed_b1.sql';
...
let _iseq = 0, _vseq = 0;
const nextItemId = () => { _iseq += 1; return `c3${pad(_iseq, 6)}-0000-0000-0000-${pad(_iseq, 12)}`; };
const nextVocabId = () => { _vseq += 1; return `b1b${pad(_vseq, 5)}-0000-0000-0000-${pad(_vseq, 12)}`; };
...
for (const u of units) {
  ...
  sql += ` ('${examId(U)}','${COURSE}','checkpoint','B1','${unitId(U)}',300,0.80,${jq({ skills: ['reading', 'listening', 'writing', 'speaking'], item_count: 10, randomize: true })})\non conflict (id) do nothing;\n`;
  ...
  u.lessons.forEach((les, li) => {
    ...
    les.items.forEach((it) => {
      ...
      const diff = Math.min(0.65, Math.max(0.35, Number(it.difficulty) || 0.48));
      rows.push(` ('${id}','${COURSE}','B1','${it.skill}','${it.type}',\n   ${dq(it.prompt, 'p')},\n   ${jq(payload)}, ${jq(it.correct)},\n   ${diff.toFixed(2)}, ARRAY['unidad${U}','${topic}','${it.skill}'])`);
    });
  });
  ...
  if (u.vocab && u.vocab.length) {
    const vr = u.vocab.slice(0, 16).map((v, k) => ` ('${nextVocabId()}','${COURSE}',${dq(v.word, 'p')},${dq(v.translation, 'p')},${500 + U * 20 + k},'${slug(v.pos || 'word')}')`);
