# -*- coding: utf-8 -*-
"""ESTUDIAR · E-2 (ALEMÁN B1+B2) — verificación con CLIENTE REAL (JWT):
...
def main():
    ...
    tok, uid = V.mk_user('studye2deb@test.jezici.dev')
    ...
    check(t is not None and t.get('cefr_level') == 'B1', 'de U13 (B1) devuelve sesión de estudio')
    ...
    check(all(c == 200 for c in codes), 'audio de U13 HEAD 200 -> %s' % codes)
    ...
    check(t22.get('cefr_level') == 'B2' and all(c == 200 for c in codes22),
          'de U22 (B2) sirve sesión y audio HEAD 200 -> %s' % codes22)
    ...
    check(bad == 0, 'las %d respuestas correctas de alemán B1/B2 se ACEPTAN (0 castigadas)' % total)
    ...
    de1 = V.rpc(tok, 'get_study_theory', {'p_unit_id': by_order[1]['id']})
    check(de1 is not None and de1.get('cefr_level') == 'A1',
          'REGRESIÓN: de U1 (tanda A1+A2) sigue sirviendo su sesión')
