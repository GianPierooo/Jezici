# -*- coding: utf-8 -*-
"""Verifica el nivel B2 NUEVO de un curso (es→fr / es→it) con cliente REAL (JWT):
  ...
  2) AISLAMIENTO: 0 lesson_items cruzan los 4 cursos; un usuario default(en) NO recibe
     ítems B2 del curso nuevo (start_practice).
...
def main():
    ...
    a2 = json.loads(run("select id,type,skill,correct_answer,payload from content_items "
                        f"where course_id='{C}' and cefr_level='B2' and not (type in ('dictation','guided_writing'));")[1])
    ck(f'{code}: hay contenido B2 sembrado', len(a2) >= 60, f"items A2={len(a2)}")

    tok, uid = mk_user(f'verify_{code}b2_probe@jezici.test')
    ...
    ck(f'{code}: determinista B2 correctos aceptados', ok == tested, f"{ok}/{tested}")
    ck(f'{code}: determinista B2 distractores rechazados', bad == tested, f"{bad}/{tested}")

    # ── 2) Aislamiento: default(en) NO recibe B2 del curso nuevo. ────────────
    tok2, uid2 = mk_user(f'verify_{code}b2_ctrl@jezici.test')
    ...
    if ids2:
        ...
        leak = C in cs
    ck(f'{code}: usuario default(en) NO recibe B2 del curso nuevo', not leak)
    ...
    rpc(tok, 'create_plan', {'p_coach_style': 'suave', 'p_intensity': 2, 'p_current_level': 'A1',
        'p_goal_level': 'B2', 'p_daily_minutes': 10, 'p_days_per_week': 5, 'p_motive': 'Viajes',
    ...
    for u in units:
        ...
        if not walk_ok:
            break
    ck(f'{code}: camina A1→B2 completo (24 unidades, cliente real)', walk_ok and reached[-1] == 24, f"llego a U{reached[-1] if reached else '-'}")
    ck(f'{code}: checkpoint U18 (ultima B1) DESBLOQUEA U19 (primera B2)', u7_unlocked_after_u6 is True)
    ...
    done_a2 = json.loads(run("select count(*) c from user_lesson_progress ulp "
                             ...
                             f"where ulp.user_id='{uid}' and u.course_id='{C}' and u.cefr_level='B2' "
    ...
    total_a2 = json.loads(run("select count(*) c from lessons le join units u on u.id=le.unit_id "
                              f"where u.course_id='{C}' and u.cefr_level='B2';")[1])[0]['c']
    ck(f'{code}: TODAS las lecciones B2 completadas (cliente real)', int(done_a2) == int(total_a2) and int(total_a2) >= 30, f"{done_a2}/{total_a2}")

    # ── 4) Audio A2 HEAD 200. ────────────────────────────────────────────────
    au = json.loads(run("select payload->>'audio_url' u from content_items "
                        f"where course_id='{C}' and cefr_level='B2' and type in ('listening','speaking_read_aloud') and payload ? 'audio_url';")[1])
    urls = [r['u'] for r in au if r['u']]
    ok200 = sum(1 for u in urls if head(u) == 200)
    ck(f'{code}: audio B2 HEAD 200', ok200 == len(urls), f"{ok200}/{len(urls)}")
