# -*- coding: utf-8 -*-
"""ESTUDIAR · E-2 (ALEMÁN A1+A2) — verificación con CLIENTE REAL (JWT):
...
def main():
    ...
    units = I.run("""select id, order_index from units
                     where course_id='%s' and order_index in (1,7,12,25)
                     order by order_index;""" % DE)
    by_order = {u['order_index']: u for u in units}

    tok, uid = V.mk_user('studye2de@test.jezici.dev')
    V.rpc(tok, 'submit_age_gate', {'p_birth_year': 1990})
    V.rpc(tok, 'set_active_course', {'p_course_id': DE})
    V.rpc(tok, 'create_plan', {
        'p_coach_style': 'suave', 'p_intensity': 3, 'p_current_level': 'A1',
        'p_goal_level': 'B1', 'p_daily_minutes': 20, 'p_days_per_week': 5,
    ...
    check(t is not None and t.get('cefr_level') == 'A1', 'de U1 (A1) devuelve sesión de estudio')
    ...
    check(all(e.get('text') for e in t['examples']),
          'los ejemplos traen la clave CANÓNICA text (alemán)')
    codes = [head(e.get('audio_url') or '') for e in t['examples']]
    check(all(c == 200 for c in codes), 'audio de U1 HEAD 200 -> %s' % codes)
    t12 = V.rpc(tok, 'get_study_theory', {'p_unit_id': by_order[12]['id']})
    codes12 = [head(e.get('audio_url') or '') for e in (t12.get('examples') or [])]
    check(t12.get('cefr_level') == 'A2' and all(c == 200 for c in codes12),
          'de U12 (A2) sirve sesión y audio HEAD 200 -> %s' % codes12)
    ...
    bank = I.run("""
      select st.unit_order, it->>'id' id, it->>'answer' ans, it->>'type' ty
       ...
       where st.course_id='%s' order by st.unit_order;""" % DE)
    by_unit = {}
    for b in bank:
        by_unit.setdefault(b['unit_order'], []).append(b)
    total = bad = 0
    for uo, items in by_unit.items():
        u = I.run("select id from units where course_id='%s' and order_index=%d;" % (DE, uo))[0]
        r = V.rpc(tok, 'submit_study_quiz', {
            'p_unit_id': u['id'],
            'p_answers': [{'id': i['id'], 'answer': i['ans']} for i in items]})
        total += r['graded']
        bad += r['graded'] - r['correct']
    check(bad == 0, 'las %d respuestas correctas de alemán se ACEPTAN (0 castigadas)' % total)

    # ── 4 · MAYÚSCULAS aceptadas · basura rechazada ──
    # .upper() convierte ß en SS (otra grafía), así que se elige una respuesta sin ß
    cand = [q for q in quiz
            if 'ß' not in ([b for b in bank if b['id'] == q['id']] or [{'ans': 'ß'}])[0]['ans']]
    q1 = cand[0] if cand else quiz[0]
    ...
    if acc_items:
        ...
        for b in acc_items:
            ...
            sinac.append((b, unicodedata.normalize(
                'NFC', ''.join(c for c in d if unicodedata.category(c) != 'Mn'))))
        u = I.run("select id from units where course_id='%s' and order_index=%d;"
                  % (FR, sinac[0][0]['unit_order']))[0]
        envio = [(b, p) for b, p in sinac
                 if b['unit_order'] == sinac[0][0]['unit_order']]
        ids = {b['id'] for b, _ in envio}
        rr = V.rpc(tok, 'submit_study_quiz', {
            'p_unit_id': u['id'],
            'p_answers': [{'id': b['id'], 'answer': p} for b, p in envio]})
        mios = [x for x in rr['results'] if x['id'] in ids]
        check(bool(mios) and all(x['correct'] for x in mios),
              'escribir SIN UMLAUT no castiga (%s)'
              % ', '.join('%s->%s' % (b['ans'], p) for b, p in sinac[:3]))
    ...
    check(t25 is None, 'de U25 (C1, sin tips) sigue null -> "teoría en camino"')
