...
NL = '--nl' in sys.argv                            # neerlandes: A1-B2 en UNA tanda (24 temas)
IT = '--it' in sys.argv and not NL                 # italiano: A1-B2 en UNA tanda (24 temas)
DE = ('--de' in sys.argv or '--de2' in sys.argv) and not (IT or NL)
DE2 = '--de2' in sys.argv
FR = ('--fr' in sys.argv or '--fr2' in sys.argv) and not (DE or IT or NL)
FR2 = '--fr2' in sys.argv
PT = ('--pt' in sys.argv or '--pt2' in sys.argv) and not (FR or IT or NL)
PT2 = '--pt2' in sys.argv
LANG = 'nl' if NL else 'it' if IT else 'de' if DE else 'fr' if FR else ('pt' if PT else 'en')  # clave del ejemplo + tl del TTS
COURSE = COURSE_NL if NL else COURSE_IT if IT else COURSE_DE if DE else COURSE_FR if FR else (COURSE_PT if PT else COURSE_EN)
DATA_ONLY = BATCH2 or PT or FR or DE or IT or NL         # la tabla y las RPCs viven desde mig 178
SRC = os.path.join(
    HERE, '_study_nl' if NL else '_study_it' if IT else ('_study_de2' if DE2 else '_study_de') if DE else ('_study_fr2' if FR2 else '_study_fr') if FR else ('_study_pt' if PT else ('_study_en2' if BATCH2 else '_study_en')),
    '_clean.json')
MIG = ('20260722120190_study_theory_nl.sql' if NL else
       '20260721120188_study_theory_it.sql' if IT else
