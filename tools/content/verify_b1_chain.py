# -*- coding: utf-8 -*-
"""Verifica el nivel B1 NUEVO de un curso (es→fr / es→it) con cliente REAL (JWT):
  ...
  2) AISLAMIENTO: 0 lesson_items cruzan los 4 cursos; un usuario default(en) NO recibe
     ítems B1 del curso nuevo (start_practice).
...
def main():
    ...
    a2 = json.loads(run("select id,type,skill,correct_answer,payload from content_items "
                        f"where course_id='{C}' and cefr_level='B1' and not (type in ('dictation','guided_writing'));")[1])
    ck(f'{code}: hay contenido B1 sembrado', len(a2) >= 60, f"items A2={len(a2)}")

    tok, uid = mk_user(f'verify_{code}b1_probe@jezici.test')
    ...
    ck(f'{code}: determinista B1 correctos aceptados', ok == tested, f"{ok}/{tested}")
    ck(f'{code}: determinista B1 distractores rechazados', bad == tested, f"{bad}/{tested}")

    # ── 2) Aislamiento: default(en) NO recibe B1 del curso nuevo. ────────────
    tok2, uid2 = mk_user(f'verify_{code}b1_ctrl@jezici.test')
    ...
    if ids2:
        ...
        leak = C in cs
    ck(f'{code}: usuario default(en) NO recibe B1 del curso nuevo', not leak)
    ...
    for u in units:
        ...
        if not walk_ok:
            break
    ck(f'{code}: camina A1→B1 completo (18 unidades, cliente real)', walk_ok and reached[-1] == 18, f"llego a U{reached[-1] if reached else '-'}")
    ck(f'{code}: checkpoint U12 (ultima A2) DESBLOQUEA U13 (primera B1)', u7_unlocked_after_u6 is True)
    ...
    done_a2 = json.loads(run("select count(*) c from user_lesson_progress ulp "
                             ...
                             f"where ulp.user_id='{uid}' and u.course_id='{C}' and u.cefr_level='B1' "
    ...
    total_a2 = json.loads(run("select count(*) c from lessons le join units u on u.id=le.unit_id "
                              f"where u.course_id='{C}' and u.cefr_level='B1';")[1])[0]['c']
    ck(f'{code}: TODAS las lecciones B1 completadas (cliente real)', int(done_a2) == int(total_a2) and int(total_a2) >= 30, f"{done_a2}/{total_a2}")

    # ── 4) Audio A2 HEAD 200. ────────────────────────────────────────────────
    au = json.loads(run("select payload->>'audio_url' u from content_items "
                        f"where course_id='{C}' and cefr_level='B1' and type in ('listening','speaking_read_aloud') and payload ? 'audio_url';")[1])
    urls = [r['u'] for r in au if r['u']]
    ok200 = sum(1 for u in urls if head(u) == 200)
    ck(f'{code}: audio B1 HEAD 200', ok200 == len(urls), f"{ok200}/{len(urls)}")
