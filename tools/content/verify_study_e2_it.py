# -*- coding: utf-8 -*-
"""ESTUDIAR · E-2 (ITALIANO A1–B2 COMPLETO) — verificación con CLIENTE REAL (JWT):
  ...
  · AISLAMIENTO CUÁDRUPLE: en, pt, fr y de siguen sirviendo cada uno SU contenido;
  · techo honesto: U25 (C1, sin tips) sigue null → "teoría en camino".
uso: python verify_study_e2_it.py
"""
import unicodedata
import urllib.error
...
def main():
    ...
    units = I.run("""select id, order_index from units
                     ...
                     order by order_index;""" % IT)
    by_order = {u['order_index']: u for u in units}

    tok, uid = V.mk_user('studye2it@test.jezici.dev')
    V.rpc(tok, 'submit_age_gate', {'p_birth_year': 1990})
    V.rpc(tok, 'set_active_course', {'p_course_id': IT})
    ...
    check(t is not None and t.get('cefr_level') == 'A1', 'it U1 (A1) devuelve sesión de estudio')
    ...
    check(all(e.get('text') for e in t['examples']),
          'los ejemplos traen la clave CANÓNICA text (italiano)')
    ...
    for uo, lvl in ((7, 'A2'), (13, 'B1'), (22, 'B2')):
        ...
        check(tx.get('cefr_level') == lvl and all(c == 200 for c in cx),
              'it U%d (%s) sirve sesión y audio HEAD 200 -> %s' % (uo, lvl, cx))
    ...
    ids = {u['order_index']: u['id'] for u in I.run(
        ...
        "and order_index between 1 and 24;" % IT)}
    total = bad = 0
    ...
    check(bad == 0, 'las %d respuestas correctas de italiano A1-B2 se ACEPTAN (0 castigadas)' % total)
    ...
    mio = frase(t)
    for cid, nom in ((EN, 'INGLÉS'), (PT, 'PORTUGUÉS'), (FR, 'FRANCÉS'), (DE, 'ALEMÁN')):
    ...
    check(t25 is None, 'it U25 (C1, sin tips) sigue null -> "teoría en camino"')
