// Genera la migración SQL del currículo B2 (Unidades 19–24) a partir del JSON
// autorado (b2_units.json, salida del workflow b2-author-content), normalizando
// al formato del grader y validando ANTES de emitir. Clon de gen_b1.mjs con:
//  · cefr_level='B2', order_index = U (19..24).
//  · ids propios (c4NNNNNN / b2bNNNNN), sin colisión con A1/A2/B1.
//  · dificultad B2 en [0.45, 0.80] (más exigente que B1).
//  · content_items con ON CONFLICT DO UPDATE (idempotente → re-sembrar aplica fixes).
import fs from 'node:fs';

const IN = process.argv[2] || 'C:/Users/gianp/Desktop/Jezici/tools/content/b2_units.json';
const OUT = process.argv[3] || 'C:/Users/gianp/Desktop/Jezici/supabase/migrations/20260619100045_seed_b2.sql';
...
let _iseq = 0, _vseq = 0;
const nextItemId = () => { _iseq += 1; return `c4${pad(_iseq, 6)}-0000-0000-0000-${pad(_iseq, 12)}`; };
const nextVocabId = () => { _vseq += 1; return `b2b${pad(_vseq, 5)}-0000-0000-0000-${pad(_vseq, 12)}`; };
...
// ── Normaliza un ítem autorado (campos planos) al formato del grader ─────────
function normalizeItem(it) {
  const t = it.type, skill = it.skill;
  if (t === 'multiple_choice' || t === 'true_false') {
    return { type: t, skill, prompt: it.prompt, payload: { options: it.options }, correct: { value: it.value } };
  }
  if (t === 'match') {
    const pairs = it.pairs || [];
    return { type: 'match', skill, prompt: it.prompt, payload: { pairs: pairs.map(([en, es]) => ({ en, es })) }, correct: { pairs } };
  }
  if (t === 'cloze') {
    return { type: 'cloze', skill, prompt: it.prompt, payload: { text: it.text }, correct: { value: it.value, accepted: it.accepted && it.accepted.length ? it.accepted : [it.value] } };
  }
  if (t === 'translation') {
    return { type: 'translation', skill, prompt: it.prompt || `Traduce: "${it.source}".`, payload: { source: it.source }, correct: { value: it.value, accepted: it.accepted && it.accepted.length ? it.accepted : [it.value] } };
  }
  if (t === 'word_bank') {
    const seq = it.sequence || [];
    return { type: 'word_bank', skill, prompt: it.prompt, payload: { tiles: it.tiles }, correct: { value: seq.join(' '), sequence: seq } };
  }
  if (t === 'reorder') {
    return { type: 'reorder', skill, prompt: it.prompt, payload: { tiles: it.tiles }, correct: { value: it.value } };
  }
  if (t === 'listening') {
    return { type: 'listening', skill, prompt: it.prompt || 'Escucha y elige lo que oíste.', payload: { options: it.options, say: it.value }, correct: { value: it.value } };
  }
  if (t === 'speaking_read_aloud') {
    return { type: 'speaking_read_aloud', skill, prompt: it.prompt || 'Lee en voz alta:', payload: { text: it.text }, correct: { expected: it.text } };
  }
...
for (const u of units) {
  const U = u.U;
  u.lessons.forEach((les, li) => les.items.forEach((rawIt, ii) => {
    const it = normalizeItem(rawIt);
    les.items[ii] = it; // reemplaza por el normalizado
  ...
  lessonRows.push(` ('${lessonId(U, u.lessons.length + 1)}','${unitId(U)}',${u.lessons.length + 1},${dq('🏁 Checkpoint Unidad ' + U, 'p')},${dq(u.checkpoint_description || 'Cronometrado · 4 habilidades · 80%.', 'p')},'checkpoint',40)`);
  ...
  sql += ` ('${examId(U)}','${COURSE}','checkpoint','B2','${unitId(U)}',300,0.80,${jq({ skills: ['reading', 'listening', 'writing', 'speaking'], item_count: 10, randomize: true })})\non conflict (id) do nothing;\n`;
  ...
  u.lessons.forEach((les, li) => {
    ...
    les.items.forEach((it) => {
      ...
      const diff = Math.min(0.80, Math.max(0.45, Number(it.difficulty) || 0.60));
      rows.push(` ('${id}','${COURSE}','B2','${it.skill}','${it.type}',\n   ${dq(it.prompt, 'p')},\n   ${jq(payload)}, ${jq(it.correct)},\n   ${diff.toFixed(2)}, ARRAY['unidad${U}','${topic}','${it.skill}'])`);
    });
  });
  ...
  if (u.vocab && u.vocab.length) {
    const vr = u.vocab.slice(0, 16).map((v, k) => ` ('${nextVocabId()}','${COURSE}',${dq(v.word, 'p')},${dq(v.translation, 'p')},${900 + U * 20 + k},'${slug(v.pos || 'word')}')`);
