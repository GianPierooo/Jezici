...
- Desviaciones:
  ...
  - [P1] ✅ **FONDO REHECHO (2026-07-09):** la escenografía se veía como **franjas de color sueltas** en Android — porque posicionaba los set-pieces por FRACCIÓN de la altura del contenido (variable, 2500–5200px en mapas largos) → las bandas (mar, montañas) se separaban con enormes huecos de degradado. Ahora se PORTA 1:1 el SVG del mockup con **anclaje absoluto**: escenografía LEJANA (sol, montañas nevadas, nubes, costa con velero) anclada ARRIBA en px; PRIMER PLANO (5 colinas contiguas + pinos + ciudad con ventanas) anclado ABAJO en px; el MEDIO es puro **degradado vertical de 8 paradas** (mockup: morado-ciudad→azul/cian-cielo→crema-cima) → paisaje cohesivo a cualquier altura, **sin franjas**, sendero limpio encima. Nubes suaves semitransparentes rellenan el cielo medio en mapas altos; velo superior (fadeUp) funde la cima. Full-bleed (llena el ancho en desktop; columna de nodos centrada dx0). Verificado con golden temporal (se ve el paisaje integrado). El globo "EXAMEN · UNIDAD N" se bajó al hueco bajo el portal (ya no sobre el arco).
